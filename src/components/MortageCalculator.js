...
function MortageCalculator() {
  ...
  const calculate = () => {
    ...
    if (
      numberofMonths.current === 0 ||
      numberofMonths.current === null ||
      interestRate.current === 0 ||
      interestRate.current === null ||
      loanAmount.current === 0 ||
      loanAmount.current === null
    ) {
      setM("NaN");
      return;
    }
    ...
    let result =
      ...
      (
        1 - Math.pow(1 + interestRate.current, numberofMonths.current * -1)
      ).toFixed(3);

    if (result > 0) setM(Math.round(result * 100) / 100);
    else {
      setM("NaN");
    }
  };
  ...
  return (
    <div>
      ...
      <div class="grid place-items-center w-screen mt-[80px]  text-4xl font-black">
        ...
        <div class="flex mb-5">${numberWithCommas(M)}</div>
