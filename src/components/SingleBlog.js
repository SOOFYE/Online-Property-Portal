...
function SingleBlog() {
    ...
    const getmyDate = (da)=>{
        ...
        let mydate = ''+date.getDate()+' '+date.toLocaleString('default', { month: 'short' })+' '+date.getFullYear();
      ...
      <article class="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
          <header class="mb-4 lg:mb-6 not-format">
              <address class="flex items-center mb-6 not-italic">
                  <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      ...
                      <div>
                          <a href="#" rel="author" class="text-xl font-bold text-gray-900 dark:text-white">{BlogData[0].fname}{" "}{BlogData[0].lname}</a>
                          <p class="text-base font-light text-gray-500 dark:text-gray-400"><time pubdate datetime="2022-02-08" title="February 8th, 2022">Published under -  {BlogData[0].category}</time></p>
                          <p class="text-base font-light text-gray-500 dark:text-gray-400"><time pubdate datetime={BlogData[0].writtendate} title="February 8th, 2022">Published on -  {getmyDate(BlogData[0].writtendate)}</time></p>
                      </div>
                  </div>
              </address>
              <h1 class="mb-4 text-3xl font-extrabold leading-tight text-gray-900 lg:mb-6 lg:text-4xl dark:text-white">{BlogData[0].title}</h1>
          </header>
          <p class="lead mb-10 -mt-3 text-lg font-light italic">{BlogData[0].metatitle}</p>
          <LazyLoadImage
              alt="Les Paul"
              src={BlogData[0].imageurl}
              class="aspect-square w-full  h-[300px] mb-10 rounded-xl object-cover"
            />
          <p class="lead mb-10 -mt-3 text-md font-normal text-justify">{BlogData[0].content}</p>
          ...
          <section class="not-format">
              ...
              <form class="mb-6" onSubmit={handleSubmit}>
                  ...
                  {(postComment)?(<button type="submit"
                      ...
                      <footer class="flex justify-between items-center mb-2">
                          <div class="flex items-center">
                              ...
                              <p class="text-sm text-gray-600 dark:text-gray-400"><time pubdate datetime="2022-02-08"
                                  title="February 8th, 2022">{getmyDate(value.daate)}</time></p>
                          </div>
