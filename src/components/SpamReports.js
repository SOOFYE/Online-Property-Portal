...
function SpamReports({ setoption2 }) {
              ...
              <tr key={index}>
                <td class="bg-white px-4 py-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      updateSpamStatus(value.spamid, value.propertyid, "Spam")
                    }
  ...
  ) : report.length === 0 && report !== undefined ? (
    <div>No Spam Reports</div>
  ) : (
    (report === undefined)(<div>Error Retriving Data</div>)
  );
}
